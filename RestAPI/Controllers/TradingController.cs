...
namespace RestAPI.Controllers
{
    public class TradingController : ApiController
    {
        ...
        //DNSE-1656 DNS.2022.03.1.09 API Dong bo co tuc
        [Route("dividend-order")]
        [System.Web.Http.HttpPost]
        public HttpResponseMessage updateCareby(HttpRequestMessage request)
        {
            string preFixlogSession = "dividend-order ";
            Log.Info(preFixlogSession + "======================BEGIN");
            Bussiness.modCommon.LogFullRequest(request);
            ...
            try
            {
                    ...
                    || request.Content.Headers.ContentType.MediaType.ToLower() == "application/json")
                {
                    ...
                    var result = Bussiness.TradingProcess.dividendorder(request.Content.ReadAsStringAsync().Result, ipaddress);
