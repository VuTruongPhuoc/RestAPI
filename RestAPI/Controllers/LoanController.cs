...
namespace RestAPI.Controllers
{
    public class LoanController : ApiController
    {
        ...
        [Route("loan-type/{id}")]
        [System.Web.Http.HttpGet]
        public HttpResponseMessage getLoanType(HttpRequestMessage request, string id)
        {
            string preFixlogSession = "loan-type/" + id + request.Method;
            Log.Info(preFixlogSession + "======================BEGIN");
            Bussiness.modCommon.LogFullRequest(request);
            ...
            try
            {
                if (request.Content.Headers.ContentType == null
                    || request.Content.Headers.ContentType.MediaType.ToLower() == "application/json")
                {
                    var result = Bussiness.LoanProcess.getLoanType(request.Content.ReadAsStringAsync().Result, id);
                    if (result.GetType() == typeof(Models.getLoanType) || result.GetType() == typeof(Bussiness.list))
                    {
                        var responses = Bussiness.modCommon.CreateResponseAPI(request, HttpStatusCode.OK, result);
                        Log.Info(preFixlogSession + "======================END");
                        return responses;
